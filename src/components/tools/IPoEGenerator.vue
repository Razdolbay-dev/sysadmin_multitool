...
import switchModels from '@/data/switches_ipoe'
...
const mainConfig = computed(() => {
  ...
  switch (sw.template) {
    case '3526':
      ...
      break
    case '3528':
      cfg += generate3528Config(sw, vlan, tftp)
      break
    case '3200':
...
// Функции генерации конфигураций
...
create vlan 150 tag ${vlan}
config vlan vlanid ${vlan} add untag ${sw.port_users}
config vlan vlanid ${vlan} add tag   ${sw.port_other}
...
config cpu_filter l3_control_pkt 1-${sw.port_count} state disable
...
create trusted_host network 172.17.17.0/24
create trusted_host 195.62.78.9
create trusted_host 10.1.0.33
create trusted_host 10.1.0.252

enable dhcp_local_relay
config dhcp_local_relay vlan vlanid ${vlan} state enable
config dhcp_local_relay option_82 ports ${sw.port_users} policy replace
config dhcp_local_relay option_82 ports ${sw.port_other} policy keep
`
}

const generate3528Config = (sw, vlan, tftp) => {
  return `
config pppoe circuit_id_insertion ports 1-${sw.port_uplink} state disable
config pppoe circuit_id_insertion ports ${sw.port_users} state enable
config pppoe circuit_id_insertion state enable

config traffic_segmentation 1-28 forward ${sw.port_uplink}
config traffic_segmentation ${sw.port_uplink} forward 1-28
...
create vlan 150 tag ${vlan}
config vlan vlanid ${vlan} add untag ${sw.port_users}
config vlan vlanid ${vlan} add tag   25-28
...
enable dhcp_local_relay
config dhcp_local_relay vlan ${vlan} state enable
`
}
...
// Функция генерации для DES-1228/ME
